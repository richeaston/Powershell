...
$localadmins = get-localgroupmember -Group Administrators | Where-Object { $_.PrincipalSource -ne 'Local' -and $_.objectclass -eq 'user' -and $_.name -ne '$result' -and $_.name -ne "$env:userdomain\SVC-LocalAdmins" -and $_.name -ne "$env:userdomain\Domain Admins" } | Select-Object -ExpandProperty Name
