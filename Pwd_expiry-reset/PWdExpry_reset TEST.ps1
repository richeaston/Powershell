...
foreach ($user in $accounts) {
    if (!($exclusions -contains $($user.SamAccountName))) {
        ...
        #Set-ADUser -Identity $user.SamAccountName -Replace @{pwdlastset=-1} -verbose -WhatIf
        set-aduser -Identity $user.SamAccountName -PasswordNeverExpires $False -verbose -WhatIf
        $now = get-date -Format "dd/MM/yy HH:mm:ss"
